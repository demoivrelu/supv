...
import {
  TransitionPresets, executeTransition,
} from '@vueuse/core';
...
function setNode(_id) {
  const edge = findEdge(_id);
  if (edge) {
    edge.data = {
      ...edge.data,
      isAnimating,
...
function runAnim() {
  console.log('module used...asdasd');
}
defineExpose({
  nodesData, runAnim, runAnimation, setNode, singleRun,
});
...
<template>
  ...
  <EdgeLabelRenderer v-if="isAnimating">
    ...
    > -->
      ...
      <span>
        <el-image :src='arm' class="el-img"></el-image>
        <!-- <span class="iconfont icon-robothand-copy-copy-copy-copy"></span>
        <span :style="{transform: `translate(0, 100%)`}" class="iconfont icon-kongban"></span> -->
      </span>
    </div>
  </EdgeLabelRenderer>
</template>
